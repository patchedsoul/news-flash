...
impl ProgressOverlay {
    pub fn new() -> Result<Self, Error> {
        let ui_data = Resources::get("ui/article_view_progress.ui").ok_or_else(|| format_err!("some err"))?;
        let ui_string = str::from_utf8(ui_data.as_ref())?;
        let builder = gtk::Builder::new_from_string(ui_string);
        let label: gtk::Label = builder.get_object("label").ok_or_else(|| format_err!("some err"))?;
        let parent: gtk::Box = builder.get_object("box").ok_or_else(|| format_err!("some err"))?;

        Ok(ProgressOverlay { parent, label })
    }
